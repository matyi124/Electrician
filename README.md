...
Nyisd meg a projektet Godot-ban.

Állítsd be a main2.tscn-t fő jelenetnek (ha még nem az).
...
A Editor2d.gd a következő, nagyon egyszerű adatszerkezetet használ:
...
Falak fekete vonallal (\_draw_walls)

fölöttük szövegben a hosszuk: „160.0 cm”

Ajtók kékkel, ablakok zölddel (\_draw_openings)

Éppen rajzolt fal preview pirosas vonallal (\_draw_current_wall_preview)

Szoba kitöltése halvány sárgával (\_draw_room_outline)
...
func get_room_polygon() -> PackedVector2Array: # Egyszerűen a falvégek konvex burka – elég prototípusnak
...
Ha már 3D-ben vagyunk → visszavált 2D-be (\_show_2d()).
...
Törli az előző fal- és device mesh-eket.

A polygon alapján padló mesh készül:

Geometry2D.triangulate_polygon(polygon) – háromszögekre bontás

mindegyik háromszögből SurfaceTool-lal vertexek

Minden poligon élre létrejön egy 3D fal:
...
A devices tömbből egyszerű kis dobozok készülnek (egyelőre csak demonstráció).
...
var x := orbit_distance _ sin(orbit_yaw) _ cos(orbit_pitch)
var y := orbit_distance _ sin(orbit_pitch)
var z := orbit_distance _ cos(orbit_yaw) \* cos(orbit_pitch)
...
Elmenti, hogy ajtó vagy ablak.

Ha az input mezők üresek, kitölti alapértékekkel:

ajtó: 90 / 210 / 0

ablak: 120 / 120 / 90

Fókusz a szélesség mezőre.

popup_centered() – ablak középre nyitása.
