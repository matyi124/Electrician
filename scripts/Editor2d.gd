...
# Előjeles poligon-terület (shoelace formula).
# Pozitív: óramutató járásával ellentétes, negatív: azzal megegyező irány.
func _signed_polygon_area(poly: PackedVector2Array) -> float:
	var n := poly.size()
	if n < 3:
		return 0.0
...
# Poligon pontjainak megfordítása (PackedVector2Array-n nincs reversed()).
func _reverse_polygon(poly: PackedVector2Array) -> PackedVector2Array:
	var result := PackedVector2Array()
	for i in range(poly.size() - 1, -1, -1):
		result.append(poly[i])
	return result

# Poligon pontok rendezése a súlypont körül, CCW sorrendbe.
func _sort_polygon_ccw(poly: PackedVector2Array) -> PackedVector2Array:
	if poly.size() < 3:
	...
	# Szög szerint rendezés
	tmp.sort_custom(func(a, b):
		return a["ang"] < b["ang"]
	)

	# Vissza PackedVector2Array-be
	var res := PackedVector2Array()
	for item in tmp:
		res.append(item["p"])

	# Biztosítsuk, hogy pozitív (CCW) legyen a terület
	if _signed_polygon_area(res) < 0.0:
		res = _reverse_polygon(res)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		...
		# Bal gomb – eszközök / szoba-kattintás
	...
	for from_key in adjacency.keys():
		for to_key in adjacency[from_key]:
			...
			if polygon.size() >= 4:
				...
				var area := _signed_polygon_area(polygon)
				if abs(area) > 0.1:
					if area < 0.0:
						polygon = _reverse_polygon(polygon)
					...
					for existing in polygons:
						...
						if all_close:
							...
							break
						if not is_duplicate:
							polygon = _sort_polygon_ccw(polygon)
...
func _draw_room_outlines() -> void:
	...
	for i in range(polys.size()):
		...
		var label := "Szoba " + str(i + 1)
		var font: Font = ThemeDB.fallback_font
		var label_pos := world_to_screen(center) + Vector2(-20, -10)
		draw_string(font, label_pos, label, HORIZONTAL_ALIGNMENT_LEFT, -1.0, 14.0, Color.BLACK)
